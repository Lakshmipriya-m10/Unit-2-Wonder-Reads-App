...
const Grade5Reading = () => {
  ...
  useEffect(() => {
    fetch("http://localhost:8080/api/stories")
      ...
      .then((data) => {
        ...
        const grade5Stories = data.filter(
          (story) => String(story.Grade) === "5"
        );

        console.log("Grade 5 stories:", grade5Stories);

        setStories(grade5Stories);
        setLoading(false);
      })
      .catch((error) => {
  ...
  // No Grade 5 stories
  if (stories.length === 0) {
    return <p>No Grade 5 stories found.</p>;
  }

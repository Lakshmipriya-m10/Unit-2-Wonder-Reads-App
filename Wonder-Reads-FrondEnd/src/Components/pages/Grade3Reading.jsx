...
const Grade3Reading = () => {
  ...
  useEffect(() => {
    fetch("http://localhost:8080/api/stories")
      ...
      .then((data) => {
        ...
        const grade3Stories = data.filter(
          (story) => String(story.Grade) === "3"
        );

        console.log("Grade 3 stories:", grade3Stories);

        setStories(grade3Stories);
        setLoading(false);
      })
      .catch((error) => {
  ...
  // No Grade 1 stories
  if (stories.length === 0) {
    return <p>No Grade 3 stories found.</p>;
  }

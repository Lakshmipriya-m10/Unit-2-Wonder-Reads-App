...
const Grade4Reading = () => {
  ...
  useEffect(() => {
    fetch("http://localhost:8080/api/stories",{
      ...
      .then((data) => {
        ...
        const grade4Stories = data.filter(
          (story) => String(story.Grade) === "4"
        );

        console.log("Grade 4 stories:", grade4Stories);

        setStories(grade4Stories);
        setLoading(false);
      })
      .catch((error) => {
  ...
  // No Grade 4 stories
  if (stories.length === 0) {
    return <p>No Grade 4 stories found.</p>;
  }
